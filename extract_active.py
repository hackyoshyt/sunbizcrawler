...
def link_extract(url_enter):
    ...
    while True:
        ...
        for active_company in active_company_link_list:
            ...
            agent_name = company_profile_data.xpath('//*[@id="main-content"]/div[2]/div[5]/span[2]/text()')
            
            for c_type, company, date, address in itertools.zip_longest(company_type, company_name, date_filed, address):
                for add1, add2, a_name in itertools.zip_longest(city_state_zip, address_additonal, agent_name):
                    try: 
                        date = datetime.datetime.strptime(date.strip(), '%m/%d/%Y').date()
                        if add2.isspace() == True:
                            print('Company Type: {0}'.format(c_type.strip() ))
                            print('Registered Agent Name: {0}'.format(a_name.strip() ))
                            print('Principal Address')
                            print('Company: {0} Date filed: {1} Address: {2}'.format(company.strip(), date.strftime("%m/%d/%Y"), address.strip() ))#date.strftime("%m/%d/%Y")
                            print('City: {0} State: {1} Zip Code: {2}'.format(add1[:-9].strip(), add1[-8:-6].strip(), int(add1[-5:].strip()) ))#slicing city state and zip
                        elif add2.isspace() != True:
                            print('Company Type: {0}'.format(c_type.strip() ))
                            print('Registered Agent Name: {0}'.format(a_name.strip() ))
                            print('Principal Address')
                            print('Company: {0} Date filed: {1} Address: {2}'.format(company.strip(), date.strftime("%m/%d/%Y"), address.strip()))
                            print('Address2: {0} City: {1} State: {2} Zip Code: {3}'.format(add1.strip(), add2[:-9].strip(), add2[-8:-6].strip(), int(add2[-5:].strip()) ))
                    except:
                        continue
            ...
            for address1, address2, address3 in itertools.zip_longest(mail_address, mail_address_additional, mail_address_csz):
                try:
                    if address3.isspace() == True:
                        ...
                        print('City: {0} State: {1} Zip Code: {2}'.format(address2[:-9].strip(), address2[-8:-6].strip(), int(address2[-5:].strip()) ))
                    elif address3.isspace() != True:
                        print('Mailing Address')
                        print('address1: {0} address2: {1} '.format(address1.strip(), address2.strip() ))
                        print('City: {0} State: {1} Zip Code: {2}'.format(address3[:-9].strip(), address3[-8:-6].strip(), int(address3[-5:].strip()) ))
                except:
                    continue
